...
void WakeUpIsr_SetVector(cyisraddress address)
{
    ...
    ramVectorTable[CYINT_IRQ_BASE + (uint32)WakeUpIsr__INTC_NUMBER] = address;
}
...
cyisraddress WakeUpIsr_GetVector(void)
{
    ...
    return ramVectorTable[CYINT_IRQ_BASE + (uint32)WakeUpIsr__INTC_NUMBER];
}
...
{
    *WakeUpIsr_INTC_PRIOR = priority << 5;
}
...
uint8 WakeUpIsr_GetPriority(void)
{
    ...
    priority = *WakeUpIsr_INTC_PRIOR >> 5;
